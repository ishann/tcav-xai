"""
Training script for learning a NeuralNet on COMPAS.
"""
################################################################################
## Import packages.
################################################################################
...
################################################################################
# Acknowledgement: google-research/generalized_rates/datasets/load_compas.py
# Read files in and pre-process data.
def preprocess_data(filepath="./data/compas/compas-scores-two-years.csv",
                    unawareness=True, verbose=False):
...
X, Y, demo = preprocess_data(filepath, verbose=True)

# Manually split data to keep track of demo as well.
X_trn, Y_trn, demo_trn = X[:5000,:], Y[:5000], demo[:5000,:]
...
torch.save(model.state_dict(), './data/compas_model.ckpt')

# Save actual data.
data = {"X": X_val,
        "Y": Y_val,
        "demo": demo_val}
dump(data, "./data/val_data.pkl")
